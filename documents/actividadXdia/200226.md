...
7. RunIdIncrementer: re-ejecuciones automaticas
...
> **Problema sin RunIdIncrementer:** Si ejecutas el Job dos veces con los mismos parametros, Spring Batch dice: "ya lo ejecute, no lo vuelvo a hacer". La solucion es usar `RunIdIncrementer` (ver seccion siguiente).
...
## 10b. RunIdIncrementer: re-ejecuciones automaticas

Sin configuracion adicional, Spring Batch **no permite** ejecutar el mismo Job dos veces con los mismos parametros. Esto es una proteccion para no reprocesar datos por accidente.

### El problema

```
Primera ejecucion:  parametros = {}  →  COMPLETED  ✓
Segunda ejecucion:  parametros = {}  →  "A job instance already exists"  ✗
```

### La solucion: RunIdIncrementer

```java
.incrementer(new RunIdIncrementer())
```

`RunIdIncrementer` agrega automaticamente un parametro `run.id` con un valor incremental (1, 2, 3...) en cada ejecucion. Asi cada ejecucion es unica:

```
Ejecucion 1:  parametros = {run.id=1}  →  COMPLETED  ✓
Ejecucion 2:  parametros = {run.id=2}  →  COMPLETED  ✓
Ejecucion 3:  parametros = {run.id=3}  →  COMPLETED  ✓
```

> **Clave:** Con `RunIdIncrementer` puedes ejecutar la app multiples veces sin necesidad de limpiar las tablas `BATCH_*`. Solo necesitas limpiar la tabla `empleados_procesados` si quieres evitar datos duplicados.
...
| `RunIdIncrementer` | Agrega `run.id` auto-incremental para permitir re-ejecuciones |
